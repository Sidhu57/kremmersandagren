          ...
          <div className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
              <div key={link.path} className="relative group whitespace-nowrap">
                <Link
                  to={link.path}
                  className={`relative px-3 py-2 text-sm font-semibold transition-all duration-300 rounded-full flex items-center gap-1 ${
                    location.pathname === link.path 
                      ? "text-primary bg-primary/10 visited:text-primary" 
                      : "text-black visited:text-black hover:text-primary hover:bg-primary/5"
                  }`}
                >
                  {link.name}
